...
public class CustomViewBehind extends ViewGroup {
	...
	private boolean mBgFadeEnabled;
	private final Paint mFadePaint = new Paint();
	private final Paint mBgFadePaint = new Paint();
	private float mScrollScale;
	private Drawable mShadowDrawable;
	private Drawable mSecondaryShadowDrawable;
	private int mShadowWidth;
	private float mFadeDegree;
	private float mBgFadeDegree;
	...
	public void setBgFadeEnabled(boolean b) {
		mBgFadeEnabled = b;
	}
	...
	public void setBgFadeDegree(float degree) {
		if (degree > 1.0f || degree < 0.0f)
			throw new IllegalStateException("The BehindFadeDegree must be between 0.0f and 1.0f");
		mBgFadeDegree = degree;
	}
	...
	public void drawFade(Canvas canvas, float openPercent) {
		if (!mBgFadeEnabled) return;
		final int alpha = (int) ((mBgFadeDegree) * 255 * Math.abs(openPercent));
		mBgFadePaint.setColor(Color.argb(alpha, 0, 0, 0));
		int left = 0;
		int right = 0;
		if (mMode == SlidingMenu.LEFT) {
			...
			right = mContent.getRight() + getBehindWidth();			
		} else if (mMode == SlidingMenu.LEFT_RIGHT) {
			left = mContent.getLeft() - getBehindWidth();
			right = mContent.getLeft();
			canvas.drawRect(left, 0, right, getHeight(), mBgFadePaint);
			left = mContent.getRight();
			right = mContent.getRight() + getBehindWidth();			
		}
		canvas.drawRect(left, 0, right, getHeight(), mBgFadePaint);
