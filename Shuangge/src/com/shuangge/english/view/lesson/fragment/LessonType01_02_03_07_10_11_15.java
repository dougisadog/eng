...
public class LessonType01_02_03_07_10_11_15 extends BaseLessonType {
	...
	public LessonType01_02_03_07_10_11_15(LessonFragment data, CallbackLessonType callback) {
		super(data, callback);
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mainView = inflater.inflate(R.layout.lesson_01_02_03_07_10_11_15, (ViewGroup) getActivity().findViewById(R.id.vp), false);
		...
		vto.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
			...
			public boolean onPreDraw() {
				if (!initialized) {
					...
					initialized = true;
					addContentView(calculationWH(llImgsContainer.getMeasuredWidth(), mainView.getMeasuredHeight() - 
...
//		anwserStrs = null;
	...
	private void initCurrentView(LessonData currentData) {
		...
		else if ("15A".equals(currentType)) {
			txtQuestionTop.setText(currentData.getAnswer().getContent());
			imgQuestionTopPlay.setVisibility(View.GONE);
			rlQuestionTop.setVisibility(View.VISIBLE);
			rlOptions01.setVisibility(View.INVISIBLE);
	...
	private void init07(LessonData data) {
		...
		if (null != data.getPhrases().get(0).getBlanks().getOptions2())
			anwserStrs.add(data.getPhrases().get(0).getBlanks().getOptions2()[0]);
		showAnwserStr = currentData.getPhrases().get(0).getContent();
		txtQuestionBottom.setText(showAnwserStr);
		next07(data.getPhrases().get(0).getBlanks().getOptions1());
	}
	
	private void next07(String[] strs) {
		rlOptions07.removeAllViews();
		options.clear();
...
//			android.util.Log.e("test", "new word random: " + word);
	...
	private void showOptions() {
		...
		if (currentType.indexOf("01") != -1 || currentType.indexOf("15") != -1) {
	...
	private int calculationWH(int w, int h) {
	...
	private void addContentView(int w) {
		if ("15A".equals(data.getDatas().get(0).getType())) {
			addVideo(w);
		} else {
			addImgs(w);
		}
	...
	private void addVideo(int w) {
		//TODO:暂时播放一个视频
		ov = new OptionVideo(getActivity(), data.getDatas().get(0).getAnswer().getLocalSoundPath(), w);
		llImgsContainer.addView(ov);
		ViewUtils.setLinearMargins(ov, LayoutParams.MATCH_PARENT, w, 0, 0, 0, 0);
	}
	
	private void addImgs(int w) {
		Bitmap bitmap = null;
		OptionImg img = null;
		String path = null;
    ...
    private class Wait1SecondThread extends Thread {
    	...
    	public void run() {
    		...
    		try {
    			...
    			LessonType01_02_03_07_10_11_15.this.pass = pass;
		...
		public void onClick(View v) {
			...
			if (rightAnwser.toUpperCase(Locale.getDefault()).equals(str.toUpperCase(Locale.getDefault()))) {
				...
				if ("07A".equals(currentType)) {
					showAnwserStr = showAnwserStr.replaceFirst("(_)+", rightAnwser);
					txtQuestionBottom.setText(showAnwserStr);
					next07(currentData.getPhrases().get(0).getBlanks().getOptions2());
					showOptions();
				}
