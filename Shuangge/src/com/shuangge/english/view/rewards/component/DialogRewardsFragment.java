...
public class DialogRewardsFragment extends DialogFragment1 implements OnClickListener {
	...
	private TextView txtRewardsInfo, txtClassScholarship,txtRank,txtScholarship;
	private String content;
	private FrameLayout flSubmit;
	public DialogRewardsFragment() {
		super();
	}
	
	public DialogRewardsFragment(CallBackDialogConfirm callback, String content) {
		super();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View v = inflater.inflate(R.layout.dialog_reward, null);// 得到加载view
		...
		//班级上周奖学金
		txtClassScholarship = (TextView) v.findViewById(R.id.txtClassScholarship);
		RewardsData rewardsData = GlobalRes.getInstance().getBeans().getLoginData().getRewardsData();
		Integer rewardsNum = rewardsData.getClassRewards();
		String rewardsNumStr = "￥0";
		if (null != rewardsNum) {
			rewardsNumStr = "奖学金：  ￥" + rewardsNum;
		}
		txtClassScholarship.setText(rewardsNumStr + "");
		
		//班级上周排名排名
		txtRank = (TextView) v.findViewById(R.id.txtRank);
		int rank = rewardsData.getClassRanking();
		String rankStr = "上周班级排名：  ";
		if (0 != rank) {
			rankStr = rankStr + rank;
		}
		txtRank.setText(rankStr);
		
		//个人上周奖学金
		txtScholarship = (TextView) v.findViewById(R.id.txtScholarship);
		double reward = rewardsData.getPersonRewards();
		String rewardStr = "￥" + reward;
		txtScholarship.setText(rewardStr);
