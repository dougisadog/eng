 ...
 *
 */
public class FragmentSecretFans extends BaseSecretFriendFragment implements OnClickListener, OnItemClickListener {
	...
	private void requestDatas() {
		showLoading();
		new TaskReqFans(0, new CallbackNoticeView<Void, Boolean>() {
			...
			public void refreshView(int tag, Boolean result) {
				...
				List<AttentionData> friends = GlobalRes.getInstance().getBeans().getAttentionInfoResult().getAttentions();
				if (friends.size() > 0) {
//					lastDate = msgs.get(msgs.size() - 1).getCreateDate();s
				}
				if (null == result || !result) {
		...
		//, GlobalRes.getInstance().getBeans().getLoginData().getInfoData().getLoginName());
	}
