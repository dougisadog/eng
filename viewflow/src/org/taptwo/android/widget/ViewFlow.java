...
public class ViewFlow extends AdapterView<Adapter> {
	...
	public static final int MEASURED_STATE_TOO_SMALL1 = 0x01000000;
	public static final int MEASURED_HEIGHT_STATE_SHIFT1 = 16;
	...
	private OnViewFlowTouchedListener onViewFlowTouchedListener;
	private OnViewFlowoInterceptTouchListener onViewFlowoInterceptTouchListener;
	
	public void setOnViewFlowoInterceptTouchListener(OnViewFlowoInterceptTouchListener onViewFlowoInterceptTouchListener) {
		this.onViewFlowoInterceptTouchListener = onViewFlowoInterceptTouchListener;
	}
	
	public static interface OnViewFlowoInterceptTouchListener {
	...
	public void setOnViewFlowTouchedListener(OnViewFlowTouchedListener onViewFlowTouchedListener) {
		this.onViewFlowTouchedListener = onViewFlowTouchedListener;
	}
	
	public static interface OnViewFlowTouchedListener {
		...
		switch (widthMode) {
			...
			case MeasureSpec.EXACTLY:
				if (widthSize < childWidth + widthPadding)
					widthSize |= MEASURED_STATE_TOO_SMALL1;
				break;
		}
		switch (heightMode) {
			case MeasureSpec.UNSPECIFIED:
				...
				break;
			case MeasureSpec.AT_MOST:
				heightSize = (childHeight + heightPadding) | (childState >> MEASURED_HEIGHT_STATE_SHIFT1);
				break;
			case MeasureSpec.EXACTLY:
				if (heightSize < childHeight + heightPadding)
					heightSize |= MEASURED_STATE_TOO_SMALL1;
				break;
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			if (null != onViewFlowoInterceptTouchListener)
				onViewFlowoInterceptTouchListener.onMouseDown(ev);
			/*
		...
		case MotionEvent.ACTION_MOVE:
			if (null != onViewFlowoInterceptTouchListener)
				onViewFlowoInterceptTouchListener.onMouseMove(ev);
		...
		case MotionEvent.ACTION_UP:
			if (null != onViewFlowoInterceptTouchListener)
				onViewFlowoInterceptTouchListener.onMouseUp(ev);
			if (mTouchState == TOUCH_STATE_SCROLLING) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (null != onViewFlowTouchedListener) {
				onViewFlowTouchedListener.onMouseDown(ev);
			}
			break;
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (null != onViewFlowTouchedListener) {
				onViewFlowTouchedListener.onMouseMove(ev);
			}
			...
			if (!canMoveRight && deltaX > 0) {
				return true;
			}
			
			if (!canMoveLeft && deltaX < 0) {
				return true;
			}
		...
		case MotionEvent.ACTION_UP:
			...
			if (null != onViewFlowTouchedListener) {
				onViewFlowTouchedListener.onMouseUp(ev);
			}
